...
- `wrangler.adapter.jsonc` exists only for adapter-cloudflare (it writes its bundle to that file's `main`). Never point the real `wrangler.jsonc` `main` at a source file without this split, or `pnpm build` overwrites it. Keep `assets` identical in both files.
