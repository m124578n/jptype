...
- ✅ C. 歌詞打字 `/songs`：嵌 youtube-nocookie 官方播放器（只存 11 字元影片 id）+ 使用者自貼假名歌詞（漢字會被擋下並指出行號），只存 localStorage、不送伺服器、不進榜；`/songs/[id]` 逐行打（前後行淡顯）
  ...
  - ✅ 前奏／間奏（**2026-09-11**，owner 實測「3 2 1 跟歌詞不同步」）：不再在按播放前倒數，改成按下去就播，倒數改用**影片時間**對第一句歌詞：距離超過 3 秒顯示「前奏 · 下一句 N 秒後」，3 秒內顯示 3-2-1；還沒唱到那句之前按鍵不算（提前 0.5 秒開始接受）；一句打完、下一句還很遠時同樣顯示「間奏 · 下一句 N 秒後」。`/contents/[id]` 的同步模式一起改
  - ✅ 修正讀音（**2026-09-11**，owner：「聽到一半或對時到一半發現讀音錯很麻煩」）：練習頁歌詞條與對時頁每一行都有「修正讀音」，逐 token 改漢字下的假名（純假名的行改整句），套用到同一首歌裡相同的句子；練習頁同步模式改完從影片目前位置接著跑，自由模式重來；存回帳號或本機
  - ✅ 逐字注音（**2026-09-11**，owner：「歌詞顯示漢字、假名在漢字上方」）：貼入時把 kuromoji 的斷詞對應存成 `SongLine.tokens`（localStorage；D1 存在 `content_lines.metadata`），練習時 `TypingArea` 用 `<ruby>` 把假名標在各漢字上方、進度高亮照樣逐字；假名改過而對不上就退回「假名一列 + 漢字一列」。同時修掉對時頁打點會把 `original` 丟掉的 bug（漢字消失在對時那一步）
  - ✅ 歌曲頁設計整理（**2026-09-11**，owner：「歌詞字小一點」「UI/UX 跑一遍」）：標題與模式切換同一列、影片與歌詞做成**同一張卡**（owner 選上下排：影片在上、歌詞條貼在影片下緣像字幕區，前一句 → 假名（縮小到 clamp(1.5rem, 4vw, 2.25rem)）+ 漢字 → 下一句 → 工具列），卡片寬度由視窗高度反推讓影片＋歌詞條一起在畫面內，鍵盤與快捷鍵說明在卡下方，公開設定移到最下面縮成一列
  - ✅ 版面與漢字（**2026-09-11**，owner：「字跟歌有點分離」「有漢字時顯示漢字、假名在上面」）：寬螢幕影片與目前這句並排（影片 sticky），窄螢幕影片高度限制在 45vh 讓歌詞留在畫面內；有原文的句子改成假名（打字列）在上、漢字原文大字在下，前後句也顯示原文
...
- ✅ 中高階與實用內容（**2026-09-11**，owner：「擴充課程加中高階」「N2 N1 也做」「商用書信」「網路用語」）：`@jptype/data` 新增 N4／N3／N2／N1 單字各 100、N4／N3／N2／N1 句子各 20、商用・書信 30 句、網路用語 60（全部原創或固定用語，含漢字與中文；`packages/data/test/content-advanced.test.ts` 驗證可輸入、不重複、長度）→ 課程地圖新增「中階（N4・N3）」「高階（N2・N1）」「實用：商用・書信與網路用語」三組共 10 課（課程總數 40），計時賽多 `n4` `n3` `n2` `n1` `slang` 五個題庫（排行榜與 cron 因此變 27 個榜）
...
- 2026-09-11 · **跟 AI 打字聊天並評分**（owner）：使用者用日文跟 AI 聊幾句（打字輸入），結束後 AI 給回饋與分數（文法、用詞、自然度，也可以連同打字速度／準確率）。屬 M4-5 AI 範圍，需要 LLM key；規格 §1「runtime 不呼叫外部 AI」要改成允許這一個功能（或走使用者自備 key），先記著
- 2026-09-17 · 開 R2 存 keylog（`runs/{id}.json`）：首次部署時 R2 未啟用（要綁付款方式），keylog 三處呼叫都改成 no-op；要做錯誤回放或反作弊人工複查時再開，步驟見 DECISIONS「先不開 R2」
